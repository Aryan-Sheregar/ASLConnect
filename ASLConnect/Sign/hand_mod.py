...
while True:
    ...
    # set_sym = []
    ld_point = []
    # distance between 1st and 2nd, 2nd and 3rd, 3rd and 4th fingers
    dist_list = [0, 0, 0]
    ...
    a1 = 0
    bool_list = [0, 0, 0, 0, 0]
    cv2.putText(frame, "FPS: "+str(int(fps)), (80, 80),
    ...
    if result.multi_hand_landmarks:
        for landmarks in result.multi_hand_landmarks:
            ...
            for i in range(8, 21, 4):
                j = int(i/4) - 2
                dist_from_thumb[j] = int(
                    ((ld_point[i][0] - ld_point[4][0])**2 + (ld_point[i][1] - ld_point[4][1])**2)**0.5)
            ...
            for i in range(8, 17, 4):
                j = int(i/4) - 2
                dist_list[j] = int(
                    ((ld_point[i][0]-ld_point[i+4][0])**2 + (ld_point[i][1]-ld_point[i+4][1])**2)**0.5)
    ...
    cv2.putText(frame, str(symbol), (40, 50),
